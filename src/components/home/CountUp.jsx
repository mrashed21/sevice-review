...
const CountUpSection = () => {
  const [review, setReview] = useState([]);
  const [services, setServices] = useState(0); // Changed to store total count directly
  const [users, setUsers] = useState([]);
  ...
  useEffect(() => {
    axios
      .get("http://localhost:4000/services") // Fetch services
      .then((res) => setServices(res.data.total)) // Use 'total' count from the response
      .catch((err) => console.error(err));
  }, []);
  ...
  const stats = [
    ...
    },
    {
      id: 3,
      label: "Services",
      count: services, // Use the updated total count
      inView: servicesInView,
      setInView: setServicesInView,
    },
  ];
